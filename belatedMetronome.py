...
def adjust_audio_segments(note_mappings, audio_file, video_file, output_folder, log_file="adjustment_log.txt", apply_filter_and_smooth=True):
    """
    ...
    Args:
        ...
        apply_filter_and_smooth (bool): Whether to apply high-pass filter and STFT smoothing to all segments.
    """
    ...
    with open(log_file, "w") as log:
        ...
        # Prepare containers
        final_audio_segments = []  # Store all processed audio segments
        final_video_segments = []  # Store all processed video segments
        last_end_time = 0.0
        ...
        def generate_transition_waveform(audio1, audio2, sr, duration):
            """
            Generate a transition waveform using zero-crossing alignment and interpolation.

            Args:
                audio1 (ndarray): Left-side audio segment.
                audio2 (ndarray): Right-side audio segment.
                sr (int): Sampling rate.
                duration (float): Duration of the gap in seconds.

            Returns:
                ndarray: Transition waveform.
            """
            zero_cross_audio1 = np.where(np.diff(np.sign(audio1)))[0]
            zero_cross_audio2 = np.where(np.diff(np.sign(audio2)))[0]

            if zero_cross_audio1.size > 0 and zero_cross_audio2.size > 0:
                end_idx = zero_cross_audio1[-1]
                start_idx = zero_cross_audio2[0]
                audio1 = audio1[:end_idx]
                audio2 = audio2[start_idx:]

            samples = int(sr * duration)
            if samples > 0:
                t = np.linspace(0, 1, samples)
                transition_waveform = np.stack([
                    np.interp(t, [0, 1], [audio1[-1], audio2[0]]),
                    np.interp(t, [0, 1], [audio1[-1], audio2[0]])
                ])
                return np.concatenate((audio1, transition_waveform, audio2), axis=-1)
            else:
        ...
        def stft_smooth(audio, sr, duration=0.025):
            """Apply STFT-based smoothing to the specified segment."""
            samples = len(audio)
            n_fft = 2048  # FFT size
            hop_length = n_fft // 4
            ...
            magnitude, phase = np.abs(audio_stft), np.angle(audio_stft)
            smoothed_magnitude = gaussian_filter1d(magnitude, sigma=2, axis=1)

            # Reconstruct the smoothed waveform
            smoothed_stft = smoothed_magnitude * np.exp(1j * phase)
            smoothed_audio = librosa.istft(smoothed_stft, hop_length=hop_length, length=samples)
        ...
        for i, mapping in enumerate(note_mappings):
            try:
                ...
                if start_time > last_end_time:
                    ...
                    if gap_audio.shape[1] == 0:
                        gap_duration = gap_end - gap_start
                        log.write(f"Generating AI-repaired transition waveform for gap from {gap_start} to {gap_end}.")
                        left_audio = final_audio_segments[-1][:, -1:] if final_audio_segments else np.zeros((2, 1))
                        right_audio = y[:, librosa.time_to_samples(gap_end, sr=sr):librosa.time_to_samples(gap_end + 0.1, sr=sr)]
                        gap_audio = generate_transition_waveform(left_audio[0], right_audio[0], sr, gap_duration)
                ...
                # Apply time correction for both channels
                ...
                final_audio_segments.append(adjusted_audio)
                final_video_segments.append(adjusted_video)
        ...
        try:
            ...
            final_audio = np.hstack(final_audio_segments)

            # Optionally apply high-pass filter and STFT smoothing to all segments
            if apply_filter_and_smooth:
                for ch in range(final_audio.shape[0]):
                    final_audio[ch, :] = highpass_filter(final_audio[ch, :], 120, sr)
                    final_audio[ch, :] = stft_smooth(final_audio[ch, :], sr)
