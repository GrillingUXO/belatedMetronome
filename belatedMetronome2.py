        ...
        for i, mapping in enumerate(note_mappings):
            try:
                ...
                if start_time > last_end_time:
                    ...
                    gap_audio = np.stack([apply_fade(gap_audio[0]), apply_fade(gap_audio[1])])
                ...
                # Apply fade and time correction for both channels
                adjusted_audio = np.stack([
                    librosa.effects.time_stretch(apply_fade(segment_audio[0]), rate=1 / time_correction),
                    librosa.effects.time_stretch(apply_fade(segment_audio[1]), rate=1 / time_correction)
                ])
                log.write(f"  Adjusted Segment Audio: Time Correction={time_correction}, Length={adjusted_audio.shape[1]} samples\n")

                adjusted_audio = np.stack([
                    apply_fade(adjusted_audio[0]),
                    apply_fade(adjusted_audio[1])
                ])
