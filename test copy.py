...
def adjust_audio_segments(note_mappings, audio_file, video_file, output_folder, log_file="adjustment_log.txt"):
    ...
    Optimized for faster video processing by reducing intermediate I/O operations.
    ...
    with open(log_file, "w") as log:
        ...
        for mapping in note_mappings:
            try:
                ...
                # Log match details
                log.write(f"Processing Segment:\n")
                log.write(f"  Performance Note: Start={start_time:.6f}, Duration={duration:.6f}, Pitch={perf_note['pitch']}\n")
                if ref_note:
                    log.write(f"  Matched Reference Note: Start={ref_note['start']:.6f}, Duration={ref_note['duration']:.6f}, Pitch={ref_note['pitch']}\n")
                else:
                    log.write("  Matched Reference Note: None (Gap)\n")
                log.write(f"  Match Round: {match_round}, DTW Score: {dtw_score}\n")
                log.write(f"  Time Correction: {time_correction:.6f}\n")
                log.write("-" * 50 + "\n")
                ...
                if start_time > last_end_time:
                    ...
                    if gap_duration < 0.01:  # Gap is less than 10ms
                        log.write(f"Detected small gap: start={gap_start:.6f}, duration={gap_duration:.6f}, end={gap_end:.6f}\n")
                        log.write(f"  Treating gap and segment as one segment.\n")

                        # Combine gap and segment
                        start_idx = librosa.time_to_samples(last_end_time, sr=sr)
                        ...
                        combined_audio = y[:, start_idx:end_idx]
                        combined_video = video_clip.subclip(last_end_time, end_time)

                        # Apply time correction
                        adjusted_audio = np.stack([
                            librosa.effects.time_stretch(combined_audio[0], rate=1 / time_correction),
                            librosa.effects.time_stretch(combined_audio[1], rate=1 / time_correction)
                        ])
                        adjusted_video = combined_video.fx(speedx.speedx, factor=(1 / time_correction))
                        log.write(f"  Combined and adjusted audio/video for small gap and segment.\n")
                        log.write(f"  Adjusted Segment Audio: Time Correction={time_correction:.6f}, Length={adjusted_audio.shape[1]} samples\n")
                ...
                adjusted_video = video_segment.fx(speedx.speedx, factor=(1 / time_correction))

                # Apply crossfade with the previous segment
                if len(segments_and_gaps) > 0:
                    previous_audio = segments_and_gaps[-1]["audio"]
                    previous_video = segments_and_gaps[-1]["video"]
                    crossfade_audio = np.zeros((2, crossfade_samples))
                    ...
                    corrected_video = concatenate_videoclips(
                        [previous_video, adjusted_video],
                        method="compose"
                    )

                    # Update the previous segment audio and video
                    segments_and_gaps[-1]["audio"] = corrected_audio
                    segments_and_gaps[-1]["video"] = corrected_video
        ...
        # Sort segments and gaps by start_time
        segments_and_gaps = sorted(segments_and_gaps, key=lambda x: x["start_time"])

        try:
            # Merge all audio segments (stereo)
            final_audio = np.hstack([s["audio"] for s in segments_and_gaps])
            ...
            # Merge all video segments (perform delayed I/O to reduce intermediate writes)
            final_video = concatenate_videoclips([s["video"] for s in segments_and_gaps], method="compose")
            final_video_with_audio = os.path.join(output_folder, "final_output_with_audio.mp4")
            
            # Add audio and save final output
            audio_clip = AudioFileClip(output_audio_file)
            final_video.set_audio(audio_clip).write_videofile(final_video_with_audio, codec="libx264", audio_codec="aac")
