...
def find_matching_notes(performance_notes, reference_notes, bpm):
    """
    Matches performance MIDI notes to reference MIDI notes using pitch, relative duration, relative time, and DTW for context similarity.
    ...
    Args:
        ...
        bpm (float): Beats per minute of the performance MIDI.

    Returns:
        list of dict: Matching note pairs with time and offset correction factors.
    """
    if bpm <= 0:
    ...
    def calculate_relative_metrics(notes):
        """
        Calculates relative metrics for notes, including relative duration and relative time.

        Args:
            notes (list of tuples): Notes as (start_time, pitch, duration).

        Returns:
            list: Notes with relative_duration and relative_time added.
        """
        relative_notes = []
        first_note_time = notes[0][0] if notes else 0  # Absolute time of the first note

        for start_time, pitch, duration in notes:
            # Calculate relative time: start time relative to the first note, in beats
            relative_time = (start_time - first_note_time) / seconds_per_beat

            # Calculate relative duration: note duration as a fraction of a beat
            relative_duration = duration / seconds_per_beat

            relative_notes.append((pitch, relative_duration, relative_time))

        return relative_notes

    # Preprocess performance and reference notes
    perf_rel_notes = calculate_relative_metrics(performance_notes)
    ref_rel_notes = calculate_relative_metrics(reference_notes)

    matches = []
    relative_time_tolerance = 60 / bpm  # Relative time tolerance in beats (3 seconds for given BPM)

    # Open log file
    with open("matching_log.txt", "w") as log:
        log.write("Performance-to-Reference Matching Log\n")
        log.write("=====================================\n")

        # Helper: Use DTW to evaluate context similarity
        def dtw_context_similarity(perf_idx, ref_idx):
            """
            Evaluate DTW context similarity for notes at given indices in performance and reference sequences.
            """
        ...
        # Match notes strictly
        for perf_idx, (perf_pitch, perf_duration, perf_time) in enumerate(perf_rel_notes):
            best_match = None
            best_score = float('inf')

            # Dynamic tolerance for relative duration
            dynamic_duration_tolerance = perf_duration * 0.75

            for ref_idx, (ref_pitch, ref_duration, ref_time) in enumerate(ref_rel_notes):
                if ref_pitch != perf_pitch:
                    continue  # Strict pitch matching

                # Check relative duration within dynamic tolerance
                duration_diff = abs(ref_duration - perf_duration)
                if duration_diff > dynamic_duration_tolerance:
                    continue

                # Check relative time within tolerance
                time_diff = abs(ref_time - perf_time)
                if time_diff > relative_time_tolerance:
                    continue

                # Check DTW context similarity
                context_score = dtw_context_similarity(perf_idx, ref_idx)
                if context_score < best_score:
                    best_match = (ref_idx, ref_pitch, ref_duration, ref_time, context_score)
                    best_score = context_score

            # Log matching result
            if best_match:
                ref_idx, ref_pitch, ref_duration, ref_time, dtw_score = best_match
                relative_offset = ref_time - perf_time
                time_correction = ref_duration / perf_duration if perf_duration > 0 else 1.0
                ...
                log.write(f"Matched: Performance {perf_pitch} (Dur: {perf_duration:.4f}, Time: {perf_time:.4f}) "
                          f"-> Reference {ref_pitch} (Dur: {ref_duration:.4f}, Time: {ref_time:.4f}) "
                          f"[DTW Score: {dtw_score:.4f}]\n")
            else:
                matches.append({
                    "performance_note": {
                        "start": performance_notes[perf_idx][0],
                ...
                log.write(f"Unmatched: Performance {perf_pitch} (Dur: {perf_duration:.4f}, Time: {perf_time:.4f})\n")
        ...
        for i, mapping in enumerate(note_mappings):
            try:
                ...
                if start_time > last_end_time:
                    ...
                    log.write(f"Gap detected: Order={order}, Start={gap_start:.2f}, End={gap_end:.2f}\n")
                ...
                log.write(f"Segment: Order={order}, Start={start_time:.2f}, End={end_time:.2f}, TimeCorrection={time_correction}\n")
